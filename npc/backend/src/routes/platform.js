...
// ─── Application Registry & Control Plane (Phase 5.1) ─────────────────────────
...
router.get('/applications/capabilities', appCtrl.getCapabilities);
router.get('/applications/:uuid', appCtrl.getApplicationByUuid);
router.put('/applications/:uuid', appCtrl.updateApplicationByUuid);
router.delete('/applications/:uuid', appCtrl.deleteApplicationByUuid);

// API Keys
router.post('/applications/:uuid/api-keys', appCtrl.createApiKey);
router.get('/applications/:uuid/api-keys', appCtrl.listApiKeys);
router.delete('/applications/:uuid/api-keys/:keyUuid', appCtrl.revokeApiKey);

// SDK Credentials
router.post('/applications/:uuid/sdk', appCtrl.generateSdkCredentials);
router.get('/applications/:uuid/sdk', appCtrl.getSdkCredentials);

// Webhooks
router.post('/applications/:uuid/webhooks', appCtrl.createWebhook);
router.get('/applications/:uuid/webhooks', appCtrl.listWebhooks);

// OAuth Clients
router.post('/applications/:uuid/oauth', appCtrl.createOauthClient);
router.get('/applications/:uuid/oauth', appCtrl.listOauthClients);

// Feature Flags
router.get('/applications/:uuid/feature-flags', appCtrl.listFeatureFlags);
router.put('/applications/:uuid/feature-flags', appCtrl.updateFeatureFlag);

// Analytics, Health, Logs, Rate Limits, Domains
router.get('/applications/:uuid/analytics', appCtrl.getAnalytics);
router.get('/applications/:uuid/health', appCtrl.getHealth);
router.get('/applications/:uuid/logs', appCtrl.getLogs);
router.get('/applications/:uuid/rate-limits', appCtrl.getRateLimits);
router.put('/applications/:uuid/rate-limits', appCtrl.updateRateLimits);
router.get('/applications/:uuid/domains', appCtrl.getDomains);
router.post('/applications/:uuid/domains', appCtrl.createDomain);

// Rotation & Verification helpers (APP-208, APP-209)
